...
- [internal/services/scanner.go](https://github.com/songloft-org/songloft/blob/main/internal/services/scanner.go) -- 文件扫描器（Walker、排除规则、软链接去环）
- [internal/services/metadata.go](https://github.com/songloft-org/songloft/blob/main/internal/services/metadata.go) -- 音频元数据提取（tag + ffprobe 双通道）
- [internal/services/song_service.go](https://github.com/songloft-org/songloft/blob/main/internal/services/song_service.go) -- 歌曲服务（ScanAndImport、远程歌曲、歌曲整理）
- [internal/services/fingerprint.go](https://github.com/songloft-org/songloft/blob/main/internal/services/fingerprint.go) -- Chromaprint 音频指纹计算
- [internal/services/auto_scan.go](https://github.com/songloft-org/songloft/blob/main/internal/services/auto_scan.go) -- 自动扫描定时调度
- [internal/services/scan_progress.go](https://github.com/songloft-org/songloft/blob/main/internal/services/scan_progress.go) -- 扫描进度追踪与状态机
- [internal/services/song_file_writer.go](https://github.com/songloft-org/songloft/blob/main/internal/services/song_file_writer.go) -- Tag 写入（原子回写音频文件）
- [internal/handlers/scan.go](https://github.com/songloft-org/songloft/blob/main/internal/handlers/scan.go) -- 扫描相关 HTTP handler
- [internal/handlers/music.go](https://github.com/songloft-org/songloft/blob/main/internal/handlers/music.go) -- 歌曲管理 HTTP handler
- [internal/app/app.go](https://github.com/songloft-org/songloft/blob/main/internal/app/app.go) -- 支持格式白名单初始化
...
8. [歌曲文件整理](#8-歌曲文件整理)
9. [扫描进度追踪](#9-扫描进度追踪)
10. [歌曲下载（remote → local）](#10-歌曲下载remote--local)
...
1. **去重链完整性**：`UpsertRemote` 按 `(plugin_entry_path, dedup_key)` 查找已有歌曲。若下载时清空 `plugin_entry_path`，后续重新导入同一歌单时 `FindSongByDedupKey` 无法命中已下载的歌曲，会创建重复记录。再次下载该重复记录时，两行的 `(plugin_entry_path="", dedup_key)` 撞上唯一索引，触发 `UNIQUE constraint failed`。
2. **UpsertRemote 已有保护**：`UpsertRemote` 命中已有行且 `type=local` 时，仅复用 ID 不覆盖任何字段，不会用远程入参污染本地化后的元数据。
3. **安全性**：`IsPluginSourced()` 要求 `PluginEntryPath` **且** `SourceData` 同时非空才返回 `true`。下载已清空 `SourceData`，因此所有依赖 `IsPluginSourced()` 的远程歌曲路径（源编排、元数据探测、缓存拉取）均不会被误触发。
