...
- Path style: `/settings/<kebab-case-name>` (e.g. `/settings/hls-proxy`, `/settings/music-path`, `/settings/http-proxy`)
...
- Core service `SongDownloader.Download`: acquire the audio (copy directly on cache hit, otherwise download synchronously) → path-template rendering → optional metadata embedding (all supported formats) → update DB (type=local)
